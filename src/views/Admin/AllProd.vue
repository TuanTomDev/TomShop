      ...
      <b-modal id="modal-center" centered title="Add New Product">
        <b-form @submit.prevent="AddProd" class="py-3">
          ...
          <b-form-input
            ...
            required
          ></b-form-input>
          <select class="form-control" id="sex" v-model="prod.sex">
            <option value="bold" disabled selected>--Select sex--</option>
...
export default {
  ...
  methods: {
    async EditCategory(catid) {
      const washingtonRef = doc(db, "categories", catid);
      await updateDoc(washingtonRef, {
        amount: increment(1),
      });
    },
    async AddProd() {
      const washingtonRef = doc(db, "categories", this.categoryId);
      await updateDoc(washingtonRef, {
        amount: increment(1),
      });
      addDoc(collection(db, `categories/${this.categoryId}/products`), {
        ...
        .then(() => {
          // asyEditCategory(this.categoryId);
